...
func OnPatternEnter():
	...
	CallingFSM.AnimPlayback.travel("SwordAttack1")
...
func PatternProcess(delta):
	...
	if CallingFSM.PlayerMovementManager.CheckAttackAvailable() == true:
		...
		if (CallingFSM.HeldItemPlaceholder==null or CallingFSM.HeldItemPlaceholder.ItemInUse.AttackPatternNode==null):
			return
		if  CallingFSM.HeldItemPlaceholder.ItemInUse.AttackPatternNode.GroundAttack2!=null:
			CallingState.OnRequestStateChange.emit(CallingState.StateName, Enums.PlayerState.GroundAttack2)
			return
